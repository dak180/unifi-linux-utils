...
function scriptTest() {
	...
	local localScriptMD5="$(md5sum "${localScripPth}/${scriptNme}" 2> /dev/null | cut -d ' ' -f 1)"
	local remotScriptMD5="$(ssh "${ssh_Address}" "md5sum '${remoteScripPth}/${scriptNme}' 2> /dev/null" | cut -d ' ' -f 1)"

	local localConfigMD5="$(md5sum "${configFile}" | cut -d ' ' -f 1)"
	local remotConfigMD5="$(ssh "${ssh_Address}" "md5sum '${remoteScripPth}/${configName}' 2> /dev/null" | cut -d ' ' -f 1)"
	...
	if [ -z "${localScriptMD5}" ]; then
		...
		scp -Bqp "${localScripPth}/${scriptNme}" "${ssh_Address}:${remoteScripPth}/${scriptNme}" || { echo "Unable to copy the script" >&2; exit 1; }
	fi

	# Copy the config file if needed
	if [ ! "${localConfigMD5}" = "${remotConfigMD5}" ]; then
		scp -Bqp "${configFile}" "${ssh_Address}:${remoteScripPth}/${configName}" || { echo "Unable to copy the config" >&2; exit 1; }
	fi
...
function certTransfer() {
	...
	scp -Bqp "${localPRIV_KEY}" "${ssh_Address}:${PRIV_KEY}"  || { echo "Unable to copy ${localPRIV_KEY}" >&2; exit 1; }

	scp -Bqp "${localSIGNED_CRT}" "${ssh_Address}:${SIGNED_CRT}"  || { echo "Unable to copy ${localSIGNED_CRT}" >&2; exit 1; }

	scp -Bqp "${localCHAIN_FILE}" "${ssh_Address}:${CHAIN_FILE}"  || { echo "Unable to copy ${localCHAIN_FILE}" >&2; exit 1; }
}
...
inPlaceCertMD5="$(ssh "${ssh_Address}" "cat ${UNIFI_DIR}/certs.md5 2> /dev/null"  | cut -d ' ' -f 1 | md5sum /dev/stdin | cut -d ' ' -f 1)"
localCertMD5="$(md5sum "${localPRIV_KEY}" "${localSIGNED_CRT}" "${localCHAIN_FILE}" | cut -d ' ' -f 1 | md5sum /dev/stdin | cut -d ' ' -f 1)"
